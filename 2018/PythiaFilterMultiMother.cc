...
bool PythiaFilterMultiMother::isAncestor(HepMC::GenParticle * particle, int IDtoMatch)
{
...
bool PythiaFilterMultiMother::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(processID == 0 || processID == myGenEvent->signal_process_id()) {
	...
	// find the daughters
	if (accepted & (daughterIDs.size()>0)){
	  ...
	  for ( HepMC::GenVertex::particle_iterator dau =(*p)->end_vertex()->particles_begin(HepMC::children); 
		...
		if ((*dau)->momentum().perp() < daughterMinPts[i]) continue;
		if ((*dau)->momentum().perp() > daughterMaxPts[i]) continue;
		if ((*dau)->momentum().eta()  < daughterMinEtas[i]) continue;
		if ((*dau)->momentum().eta()  > daughterMaxEtas[i]) continue;
		++good_dau;
	      }
	    }
	  }
	  if(good_dau<daughterIDs.size()) accepted=false;
	}
